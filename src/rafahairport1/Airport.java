...
public class Airport {
    ...
    //  دالة البحث عن بوابة متاحة (بدون شرط)
    ...
    // دالة البحث عن بوابة متاحة بسعة كافية (مع شرط السعة)
    public Gate findAvailableGate(int minCapacity) {
        for (Gate gate : gates) {
            if (gate.isAvailable() && gate.getCapacity() >= minCapacity) {
                return gate;
            }
        }
        return null;
    ...
    //  دالة البحث عن بوابة مناسبة (نفس الدالة السابقة)
    public Gate findSuitableGate(int requiredCapacity) {
        for (Gate gate : gates) {
